/**
 * Pure markdown line-prefix algebra.
 *
 * Every paragraph/list/quote style these commands offer is, in markdown
 * source, a LINE PREFIX. Parsing a line into
 * `indent + quote + list + heading + content` and rebuilding it turns "make
 * this a heading" into a surgical rewrite of the prefix region only — content
 * characters (and therefore the kernel's span identities, collaborative ops
 * and undo granularity) are never touched.
 *
...
/** The three list flavours the commands can apply. */
export type ListKind = "bullet" | "ordered" | "todo";

/** Paragraph style = heading level, where 0 is body text. */
 ...
 *  — a link is content, not formatting, and has its own command. */
...
export function offsetOfLine(md: string, index: number): number {
    let offset = 0;
