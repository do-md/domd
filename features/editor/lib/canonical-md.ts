 ...
 *
 * Since @do-md/core-react 0.7.0 the EditorStore class is no longer part of
 * the public surface, so headless instantiation is out. Instead the app
 * mounts a hidden scratch DOMDProvider (store only — no <DOMD/> surface,
 * hence no editor DOM) configured with the SAME parsing options as the live
 * editor, and registers its store here (see ScratchCanonicalizer in
 * editor-app). Multiple providers coexisting is proven ground —
 * /playground/live runs two side by side.
 */
